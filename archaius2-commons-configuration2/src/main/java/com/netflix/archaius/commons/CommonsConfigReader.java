...
public class CommonsConfigReader implements ConfigReader {
    ...
    private com.netflix.archaius.api.Config load(URL url) throws ConfigurationException {
        FileBasedConfigurationBuilder<FileBasedConfiguration> config = new FileBasedConfigurationBuilder(FileBasedConfiguration.class).configure(new FileBasedBuilderParametersImpl().setURL(url));

        return new CommonsToConfig(config.getConfiguration());
    }
